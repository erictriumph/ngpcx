...
### Scanner
Discovers observable facts about the machine: installed software, connected/recent devices, driver evidence, and Guidance Signals (running, startup, launchable, AppX, unlisted, taskbar-pinned, UserAssist launch evidence, experimental UserAssist focus evidence, default-handler evidence). Reports facts, never judges relevance or importance. Frozen; out of scope for this redesign except as a fixed input.
...
Guidance Signals (running, startup, launchable, AppX, unlisted, taskbar-pinned, UserAssist launch evidence, experimental UserAssist focus evidence, default-handler evidence) were collected specifically to support Workspace organization, not recommendation scoring — see CLAUDE.md, Observation Enrichment / Scanner Enrichment / UserAssist Enrichment milestones. In this design, that boundary is structural, not just a stated intent:
...
4. **Surfacing `unlisted_apps` as investigable content.** Doesn't feed the score today, by explicit design (see CLAUDE.md, Observation Enrichment). Pulling this array into the assessment surface as its own content type would directly undermine the "small handful" promise v1 exists to prove. The per-entity *boolean* Guidance Signals on already-scored apps are used; this discovery array is not yet surfaced as content. (`appx_apps` no longer belongs in this deferred item — see CLAUDE.md, Scanner Enrichment milestone: AppX apps are now merged into the same classified population as every other app, so they already appear in the existing Native/Emulated/Unsupported/Unknown tables like any other app, not as a separate surface. This item was updated, not v1's scope — nothing about the Workspace design itself changed.)
